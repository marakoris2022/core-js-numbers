 ...
 *   (0,1) (0,1)     => 0
 */
function getAngleBetweenVectors(vectX1, vectY1, vectX2, vectY2) {
  ...
  return angleBetweenVectors(vectX1, vectY1, vectX2, vectY2);
}
 ...
 *   10 => 55
 */
function getFibonacciNumber(indexY) {
  ...
  return fibonacciIterative(indexY);
}
...
function isNumber(value) {
  let answer = false;
  if (typeof value === 'number') answer = true;
  if (!Number.isFinite(value)) answer = false;
  if (Number.isNaN(value)) answer = false;
  return answer;
}
...
function getSumOfNumbers(x1, x2, x3) {
  let answer;
  if (Number.isInteger(x1 + x2 + x3)) {
    answer = x1 + x2 + x3;
  } else {
    answer = Number(x1 + x2 + x3).toFixed(1);
  }
  return answer;
}
...
function getHypotenuse(a, b) {
  ...
  // return Math.sqrt(a) * Math.sqrt(a) + Math.sqrt(b) * Math.sqrt(b);
}
